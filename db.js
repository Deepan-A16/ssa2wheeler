...
async function initDatabase() {
  try {
    ...
    const defaultEmail = process.env.RECIPIENT_EMAIL || process.env.GMAIL_USER || 'd.a162620078@gmail.com';

    const userRes = await pool.query(
      `SELECT id, username, password, email FROM users WHERE username = $1 OR username = 'ssaparking'`,
      [defaultHashedUsername]
    );

    if (userRes.rows.length === 0) {
      await pool.query(
        `INSERT INTO users (username, password, full_name, role, email) VALUES ($1, $2, 'SSA Parking Operator', 'owner', $3)`,
        [defaultHashedUsername, defaultHashedPassword, defaultEmail]
      );
    } else {
      ...
      await pool.query(
        ...
        [newUsername, newPassword, userEmail, user.id]
      );
    }

    // Ensure all existing user records have a non-null email
    await pool.query(
      `UPDATE users SET email = $1 WHERE email IS NULL OR email = ''`,
      [defaultEmail]
    );
