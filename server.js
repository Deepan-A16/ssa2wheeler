...
// Explicit routes for reset password page
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    const query = `SELECT id, username, full_name, phone, role FROM users WHERE (username = $1 OR LOWER(email) = $2) AND password = $3`;
    
    const result = await db.query(query, [hashedUsername, cleanInput, hashedPassword]);
...
app.post('/api/auth/reset-password', async (req, res) => {
  try {
    ...
    const updateRes = await db.query(`UPDATE users SET password = $1 WHERE id = $2`, [hashedPassword, resetRecord.user_id]);

    if (updateRes.rowCount === 0) {
      console.error(`⚠️ Password reset failed: User ID ${resetRecord.user_id} not found in database.`);
      return res.status(404).json({ success: false, message: 'User account not found in database.' });
    }

    console.log(`✅ Password successfully updated in database for User ID: ${resetRecord.user_id}`);
...
app.post('/api/parking/entry', async (req, res) => {
  try {
    ...
    if (!tokenNo && tokenNo !== 0) {
      return res.status(400).json({ success: false, message: 'Token Number is required to register a vehicle.' });
    }
    ...
    const parsedToken = parseInt(tokenNo, 10);
    const now = new Date();
